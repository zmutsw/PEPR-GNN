...
#LEARNING (SHOULD CAPTURE BINDING MOTIFS), INSTEAD OF DOING LLM BULLSHIT
...
model.eval()
pred = model(testdata).argmax(dim=1)
correct = (pred == testdata.x[:,1].long()).sum()
acc = int(correct) / len(testdata.x[:,1].long())
print(f'Accuracy: {acc:.4f}')
print(acc)
...
sb.histplot(pred,binwidth=1,binrange=(0,19))
...
print('accuracy by PERTEP category:')
for categ in range(19):
    cat_acc = torch.logical_and((pred == testdata.x[:,1].long()),(pred == categ)).sum()/(pred == categ).sum()
    print(categ,cat_acc)
...
#old ARGVA Implementation, doesn't consider edge weights so probably unneeded

class ARGVA(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = GCNConv(-1, 16)
        self.conv2 = GCNConv(16, 32)
    def forward(self, data):
        #want x to somehow pass forward the adjacency matrix and node info?
        x, edge_index = data.x, data.edge_index
#         x, edge_index = data.card, data.edge_index
        x = self.conv1(x, edge_index)
        x = F.relu(x)
        x = F.dropout(x, training=self.training)
        x = self.conv2(x, edge_index)
...
for epoch in range(10):
    ...
    out = model(data)
#     print(out.shape)
    loss = F.nll_loss(input=out,target=target_feat.long())
    loss.backward()
    optimizer.step()
    if epoch%10==0:
        print(dtime()-start)

# test_target_feat = testdata.x[:,1]
test_target_feat = data.card

model.eval()
pred = model(testdata).argmax(dim=1)
correct = (pred == test_target_feat.long()).sum()
acc = int(correct) / len(test_target_feat.long())
print(f'Accuracy: {acc:.4f}')
print(acc)
...
# ## SET UP GENE MAPS/VIEW                       (run function block before these two)

# In[86]:


tgrh = 'Ccdc184'
print(promol[gl.index(tgrh)])
...
#full T->E->P->R graphs with thresholding
this_graph = fgl[gx].copy()
g = this_graph
#     print(g)
#     print(list(g.nodes))
g = remove_indirect_edges(g)
#     print(list(g.nodes))
this_gene = list(g.nodes)[-1].split('\n')[0]

#remove all edges not passing pv threshold, and clean up nodes after
pthresh = -np.log10(0.05/len(pr)+10e-15)
# pthresh = -np.log10(0.05+10e-15)
weak_pv_edges = [elem[0:2] for elem in g.edges.data('pv') if elem[2] != None and elem[2] < pthresh]
g.remove_edges_from(weak_pv_edges)
g.remove_edges_from(weak_pv_edges) #must perform twice to remove both fc and pv edges
lonely_nodes = [node[0] for node in g.degree() if node[1]==0]
g.remove_nodes_from(lonely_nodes)

# set edge saturation and width to scale with fc
widths = nx.get_edge_attributes(g, 'fc')
wscale = 5
widtharr,alpharr = wisat_getter(widths)

#set cool colors for (-) fc and warm colors for (+) fc
green_val = 0.2
colorarr = color_setter(green_val,list(g.edges.data('fc')))

# define numbers of each node type
nodelist = g.nodes()
lnl = list(nodelist)
clean_lnl = lnl.copy()
clean_t = [elem for elem in lnl if elem in perts]
clean_lnl = [elem for elem in clean_lnl if elem not in clean_t]
clean_pr = [elem for elem in lnl if 'Promoter' in elem or 'RNA' in elem]
clean_e = [elem for elem in clean_lnl if elem not in clean_pr]
tnum,enum,prnum = len(clean_t),len(clean_e),len(clean_pr)

#set node positions
exscale,eyscale = 1,4
txscale,tyscale=1,4
pos = {**{lnl[tf_idx]:[0,(0.5*tnum-tf_idx-0.5)*tyscale] for tf_idx in range(tnum)},
       **{lnl[tnum+enh_idx]:[1*exscale,(0.5*enum-enh_idx-0.5)*eyscale] for enh_idx in range(enum)},
       **{clean_pr[pr_idx]:[(2+pr_idx)*exscale,0] for pr_idx in range(prnum)}}

# actual drawing functions for each aspect of the plot
# nx.draw_networkx_nodes(g,pos,nodelist=nodelist,node_size=1200,cmap=plt.cm.rainbow,node_color=range(len(g.nodes())),vmin=0,vmax=len(g.nodes()),alpha=1,ax=axs[int(iternum/colnum),iternum%colnum])
# nx.draw_networkx_edges(g,pos,edgelist = widths.keys(),width=widtharr,edge_color=colorarr,arrowstyle = '-',alpha=alpharr,ax=axs[int(iternum/colnum),iternum%colnum])
# nx.draw_networkx_labels(g,pos=pos,labels=dict(zip(nodelist,nodelist)),font_size=8,font_color=(0,0,0),ax=axs[int(iternum/colnum),iternum%colnum])

fig, ax = plt.subplots(figsize=(15,10))
nx.draw_networkx_nodes(g,pos,nodelist=nodelist,node_size=1200,cmap=plt.cm.rainbow,node_color=range(len(g.nodes())),vmin=0,vmax=len(g.nodes()),alpha=1,ax=ax)
nx.draw_networkx_edges(g,pos,edgelist = widths.keys(),width=widtharr,edge_color=colorarr,arrowstyle = '-',alpha=alpharr,ax=ax)
nx.draw_networkx_labels(g,pos=pos,labels=dict(zip(nodelist,nodelist)),font_size=8,font_color=(0,0,0),ax=ax)

plt.box(False)
plt.tight_layout()
this_fig = plt.gcf()
# ax[int(iternum/colnum),iternum%colnum].title.set_text(this_gene)
# figl.append(this_fig)
# plt.savefig('graph_'+lib+'_Fbn1_direct.pdf')
plt.show()
...
def tree_assembler(gene_name,t_split,rep):
    #define gene and gather needed indices
    tss_pos = np.where(ce_tss['gid'] == gene_name)[0]
    gene_promol = [elem for elem in promol if elem[0] in tss_pos][rep] #specifies first TSS for a given gene
    gene_enhol = [elem for elem in enhol if elem[0] in tss_pos][rep] #specifies first TSS for a given gene
...
#         print(ce_atac.var_names[ind])
    ...
    for val in range(val_num): #0=p, 1=q, 2=fc for bl & ln     0=p, 1=fc for co
...
#         return
    ...
    #sb.heatmap(data=q_tree[:,0:1],xticklabels=edge_names[0:1],yticklabels=16,ax=ax[0,0],center=np.log10(5+10**-2),vmax=2,linewidths=0,linecolor='w',cmap=coloring)
    ...
    #sb.heatmap(data=q_t[:,1:3],xticklabels=edge_names[1:3],yticklabels=1,ax=ax[0,2],center=np.log10(5+10**-2),vmax=2,linewidths=0,linecolor='w',cmap=coloring)
    ...
    #sb.heatmap(data=q_e[:,3:6],xticklabels=edge_names[3:6],yticklabels=1,ax=ax[0,1],center=np.log10(5+10**-2),vmax=2,linewidths=0,linecolor='w',cmap=coloring)
...
def color_setter(g_val,colors):
    colors = [elem for elem in colors if elem[2]!=None]
    fcs = [2**(edge[2]/wscale) - 2**-5 for edge in colors]
    for color in range(len(fcs)):
        if fcs[color]<0.5:
            fcs[color] = 0.5
        elif fcs[color]>1.5:
            fcs[color]=1.5
    color_l = [(0.5+color-1,g_val,0.5-color+1) for color in fcs]
    colorarr = np.array(color_l)
    
    return colorarr



def wisat_getter(widths):
    widtharr = np.array(list(widths.values()))
    #offset weights to give minimum width for better visual
    widtharr[widtharr>=0]+=0.1
    widtharr[widtharr<0]-=0.1
    #set saturation to increase with abs(widtharr[element])
    alpharr = np.array(list(widths.values()))/wscale
    alpharr[alpharr<0]=-alpharr[alpharr<0]
    alpharr+=0.2
    alpharr[alpharr>1]=1
    widtharr[widtharr<0] = -widtharr[widtharr<0] #for export to pdf
    
    return widtharr,alpharr

def remove_indirect_edges(g):
    #assume last two nodes are prom and RNA, and g is ordered as {{t},{e},p,r}
    t_num = 16
    all_edges = list(g.edges)
    all_nodes = list(g.nodes)
    tpr_edges = [elem[0:2] for elem in all_edges if elem[0] in all_nodes[0:t_num] and elem[1] in all_nodes[-2:]]
    er_edges = [elem[0:2] for elem in all_edges if elem[0] in all_nodes[t_num:-2] and elem[1] == all_nodes[-1]]
    indir_edges = tpr_edges+er_edges
    g.remove_edges_from(indir_edges)
    g.remove_edges_from(indir_edges) #must perform twice to remove both pv AND fc edges
    return g
